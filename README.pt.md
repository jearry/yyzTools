...
<p align="center">
  <img src="screenshots/en/01-cmd-plate-home.png" alt="Paleta de comandos">
  <img src="screenshots/en/05-dock-overview.png" alt="Dock">
  <img src="screenshots/en/26-settings.png" alt="Definições">
</p>
...
- **387 módulos de comandos** — Comandos de cinco categorias (IA, aplicações, jogos, sistema e sites) prontos a usar; basta um ficheiro JSON para adicionar os seus.
...
| **Arranque e produtividade** | Dock · Histórico da área de transferência · Estatísticas de aplicações · Captura de ecrã · Gravação de ecrã · Assistente de lembretes · Calendário permanente · Ações rápidas do sistema · Gestos do mouse · Varredura e limpeza de arquivos relâmpago · Análise de espaço em disco relâmpago · Limpeza de memória relâmpago · Otimizador de sistema relâmpago |
| **Texto e idiomas** | Super tradução · Reconhecimento de texto OCR · OCR rápido · Expressões regulares · Codificar/decodificar · Cálculo de hash · Encriptar/desencriptar · Conversão de dados · Geração de dados |
...
| **Cálculo e extensões** | Rascunho de cálculo · Código QR · SDK aberto · Browser integrado |
| **Personalização** | Papéis de parede dinâmicos · Efeitos de ambiente de trabalho · Página de navegação |
...
| `yyzScreenCap.exe`   | Captura de ecrã — região, deslocamento e fixar no ecrã |
| `yyzScreenRec.exe`   | Gravação de ecrã — câmara que segue o cursor, cliques e teclas em destaque |
| `yyzFileClean.exe`   | Varredura e limpeza de arquivos relâmpago (ImGui) |
| `yyzDiskSpace.exe`   | Análise de espaço em disco relâmpago (ImGui) |
| `yyzMemClean.exe`    | Limpeza de memória relâmpago (ImGui) |
| `yyzWinOpt.exe`      | Otimizador de sistema relâmpago (ImGui) |
...
- Componentes open source (licenças em [LICENSE](LICENSE)):
  - [`src/library/`](src/library/) —— biblioteca de utilitários partilhada (yyzlib)
  - [`src/updater/`](src/updater/) —— atualizador automático
  - [`src/yyzcmd/`](src/yyzcmd/) —— executor de comandos do sistema
  - [`src/inputhint/`](src/inputhint/) —— indicação de teclas
  - [`src/mousefinder/`](src/mousefinder/) —— localização do rato

Para reportar problemas de funcionalidades, utilize [Issues](https://github.com/jearry/yyzTools/issues).

## Licença e Agradecimentos

O yyzTools é software proprietário, de utilização gratuita. Atualmente apenas alguns componentes são open source, licenciados nos termos da **Licença MIT**. A decisão de abrir o código de outras partes ou da totalidade da aplicação será tomada conforme as circunstâncias.
