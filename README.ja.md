...
<p align="center">
  ...
  <img src="screenshots/en/26-settings.png" alt="設定">
</p>
...
- **387 個のコマンドモジュール** — AI、アプリ、ゲーム、システム、Web サイトの 5 分類のコマンドをすぐに利用できます。JSON ファイルを 1 つ追加するだけで自分で拡張できます。
...
| **ランチャーと効率化** | ドック · クリップボード履歴 · アプリ使用統計 · スクリーンショット · 画面収録 · リマインダー · 万年暦 · システムクイック操作 · マウスジェスチャー · 瞬速ファイルスキャン＆クリーン · 瞬速ディスク容量分析 · 瞬速メモリクリーナー · 瞬速システム最適化 |
| **テキストと言語** | スーパー翻訳 · 文字認識 OCR · クイック OCR · 正規表現 · エンコード・デコード · ハッシュ計算 · 暗号化・復号 · データ変換 · データ生成 |
| **ファイルとメディア** | ファイルプレビュー · 一括リネーム · 画像の一括処理 · 動画の一括処理 · PDF の一括処理 · ファイルダウンロード |
| **計算と拡張** | 計算用紙 · QR コード · オープン SDK · 内蔵ブラウザー |
| **パーソナライズ** | ライブ壁紙 · デスクトップエフェクト · ナビゲーションページ |
...
| `yyzScreenCap.exe`   | スクリーンショット——領域・スクロール・ピン留め |
| `yyzScreenRec.exe`   | 画面収録——カメラがカーソルを追従、クリック・キー表示付き |
| `yyzFileClean.exe`   | 瞬速ファイルスキャン＆クリーン（ImGui） |
| `yyzDiskSpace.exe`   | 瞬速ディスク容量分析（ImGui） |
| `yyzMemClean.exe`    | 瞬速メモリクリーナー（ImGui） |
| `yyzWinOpt.exe`      | 瞬速システム最適化（ImGui） |
...
- オープンソースコンポーネント（ライセンスの詳細は [LICENSE](LICENSE)）：
  - [`src/library/`](src/library/) —— 共有ユーティリティライブラリ yyzlib
  - [`src/updater/`](src/updater/) —— 自動アップデーター
  - [`src/yyzcmd/`](src/yyzcmd/) —— システムコマンド実行
  - [`src/inputhint/`](src/inputhint/) —— キー入力表示
  - [`src/mousefinder/`](src/mousefinder/) —— マウス位置表示

機能に関する問題の報告は [Issues](https://github.com/jearry/yyzTools/issues) までお願いします。

## ライセンスと謝辞

yyzTools はプロプライエタリソフトウェアで、無料で利用できます。現時点でオープンソース化されているのは一部コンポーネントのみで、それらは **MIT ライセンス** に従います。今後他の部分または全体をオープンソース化するかどうかは、状況に応じて判断いたします。
