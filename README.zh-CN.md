...
- **安装包**（推荐）：`yyzTools-setup-1.0.8.1800.exe` —— [GitHub Releases](https://github.com/jearry/yyzTools/releases)
...
- 开源组件（许可证详见 [LICENSE](LICENSE)）：
  - [`src/library/`](src/library/) —— 共享基础库 yyzlib
  - [`src/updater/`](src/updater/) —— 自动更新器
  - [`src/yyzcmd/`](src/yyzcmd/) —— 系统命令执行器
  - [`src/inputhint/`](src/inputhint/) —— 键盘/鼠标按键提示
  - [`src/mousefinder/`](src/mousefinder/) —— 鼠标光标定位

如需反馈功能问题，请至 [Issues](https://github.com/jearry/yyzTools/issues)。

## 许可证与致谢

yyzTools 是专有软件，可免费使用。目前仅开源了部分组件，这些开源组件遵循 **MIT 协议**。后续是否开源其他部分和全部开源，将根据情况决定。
